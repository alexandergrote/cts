...
class Chi2Importance(BaseModel):
    ...
    def get_feature_importance(self, X: pd.DataFrame, y: pd.Series) -> pd.Series:
        ...
        if result is None:
            ...
            result = pd.Series(chi2_stats, index=X.columns)
            cache_handler.write(obj=result)

        return result


class Chi2FeatSelection(BaseModel, BaseFeatureSelector):
    ...
    def _select_features_train(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        chi_2 = Chi2Importance()
        feature_importance = chi_2.get_feature_importance(X, y)
...
if __name__ == '__main__':
    ...
    result = Chi2FeatSelection(
        n_features=200
    )._select_features_train(data=data)
